                  ...
                  return (
                    <tr key={rowKey} className="hover:bg-slate-50/60">
                      ...
                      <td className="py-3 font-bold text-slate-900">
                        {product.pricing?.dailyRate ? formatCurrency(product.pricing.dailyRate) : "—"}
                      </td>
                      <td className="py-3">
                      ...
                      <td className="py-3 text-right whitespace-nowrap space-x-3">
                        <Link
                          to={productId ? `/seller/products/${productId}/edit` : "#"}
                          onClick={(event) => !productId && event.preventDefault()}
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 text-rose-600 hover:text-rose-700 disabled:opacity-40 font-semibold text-[11px]"
                        >
                    ...
                    <tr key={rowKey} className="hover:bg-slate-50/60">
                      ...
                      <td className="py-3">
                        {booking.bookingItems?.[0] && (
                          <span>
                            {formatDate(booking.bookingItems[0].startDate)} → {formatDate(booking.bookingItems[0].endDate)}
                          </span>
                        )}
                      </td>
                      <td className="py-3">
                        <span className="capitalize px-2 py-0.5 rounded-md font-bold text-[11px] bg-slate-100 text-slate-800">{booking.status}</span>
                      </td>
                      ...
                      <td className="py-3 text-right space-x-2">
                        {bookingId && booking.status === "pending" && (
                          <>
                            <button
                              onClick={() => updateStatusMutation.mutate({ bookingId, status: "confirmed" })}
                              disabled={updateStatusMutation.isPending}
                              className="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 disabled:opacity-50 font-bold text-[11px]"
                            >
                              Accept
                            </button>
                            <button
                              ...
                              disabled={updateStatusMutation.isPending}
                              className="px-2.5 py-1 rounded-lg bg-rose-50 text-rose-700 hover:bg-rose-100 disabled:opacity-50 font-bold text-[11px]"
                            >
                              Decline
                            </button>
                          </>
                        ...
                        {bookingId && booking.status === "confirmed" && (
                          <button
                            ...
                            disabled={updateStatusMutation.isPending}
                            className="px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-700 hover:bg-indigo-100 disabled:opacity-50 font-bold text-[11px]"
                          >
                        ...
                        {bookingId && booking.status === "return_requested" && (
                          <button
                            ...
                            disabled={updateStatusMutation.isPending}
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 font-bold text-[11px]"
                          >
                        ...
                        {bookingId && booking.status === "returned" && (
                          <button
                            ...
                            disabled={updateStatusMutation.isPending}
                            className="px-2.5 py-1 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 font-bold text-[11px]"
                          >
                            Complete Booking
                          </button>
                        )}
                        {bookingId && (
                          <Link to={`/bookings/${bookingId}`} className="text-slate-500 hover:underline text-[11px]">Details</Link>
                        )}
                      </td>
                    </tr>
                  );
