...
export function MessagesPage() {
  ...
  // Auto-select first conversation or handle recipient param
  useEffect(() => {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async () => {
      ...
      const recipientId =
        activeConv?.customerId === user?.id
          ? activeConv?.sellerId
          : activeConv?.customerId || initialRecipient;
  ...
  const otherParty =
    currentConv?.customerId === user?.id ? currentConv?.seller : currentConv?.customer;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden grid grid-cols-1 md:grid-cols-12 h-[750px]">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
            {loadingConversations ? (
              ...
              conversations.map((conv) => {
                const partner = conv.customerId === user?.id ? conv.seller : conv.customer;
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline justify-between mb-0.5">
                        ...
                        <span className="text-[10px] text-slate-400">
                          {formatDate(conv.lastMessageAt, "MMM d")}
                        </span>
                      </div>
                      {conv.product && (
                        <div className="text-[11px] font-medium text-indigo-600 truncate mb-1">
                          {conv.product.name}
                        </div>
                      )}
                    </div>
        ...
        {/* ─── Right: Message Thread & Input (8 cols) ──────────── */}
            ...
            <>
              ...
              <div className="p-4 bg-white border-b border-slate-200 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-slate-900">{otherParty?.name}</h3>
                    {currentConv?.product && (
                      <Link
                        to={`/products/${currentConv.product.id}`}
                        className="text-[11px] text-indigo-600 hover:underline"
                      >
                        Listing: {currentConv.product.name}
                      </Link>
                    )}
                  </div>
                </div>
